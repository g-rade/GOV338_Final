...
sandy_joined <- sandy_combos %>% 
  ...
  mutate(cross_party = ifelse(party == party2, 1, 0), storm1 = ifelse(state %in% c("PA", "MA", "NH", "WV", "WA", "MD", "DE", "RI", "CT", "NJ", "NY"), 1, 0), storm2 = ifelse(state2 %in% c("PA", "MA", "NH", "WV", "WA", "MD", "DE", "RI", "CT", "NJ", "NY"), 1, 0), storm = ifelse(storm1 == 1 & storm2 == 1, 1, 0), sharevote = ifelse(vote == vote2, 1, 0)) %>% 
...
harvey_joined <- harvey_combos %>% 
  ...
  mutate(cross_party = ifelse(party == party2, 1, 0), storm1 = ifelse(state %in% c("TX", "LA"), 1, 0), storm2 = ifelse(state2 %in% c("TX", "LA"), 1, 0), storm = ifelse(storm1 == 1 & storm2 == 1, 1, 0), sharevote = ifelse(vote == vote2, 1, 0)) %>% 
  select(-c(storm1, storm2)) %>% 
  filter(sharevote == 1)
